...
// 아래 `login_info.g.dart` 가 자동으로 생성 됨

// flutter pub run build_runner watch

/// 이 구문은 `LoginInfo` 클래스가 생성된 파일의 private 멤버들을
...
@JsonSerializable()
class LoginInfo {
  LoginInfo(this.memberToken);

  String memberToken;

  /// map에서 새로운 User 인스턴스를 생성하기 위해 필요한 팩토리 생성자입니다.
  /// 생성된 `_$LoginInfoFromJson()` 생성자에게 map을 전달해줍니다.
  /// 생성자의 이름은 클래스의 이름을 따릅니다.
  factory LoginInfo.fromJson(Map<String, dynamic> json) =>
      _$LoginInfoFromJson(json);

  /// `toJson`은 클래스가 JSON 인코딩의 지원을 선언하는 규칙입니다.
  /// 이의 구현은 생성된 private 헬퍼 메서드 `_$LoginInfoFromJson`을 단순히 호출합니다.
